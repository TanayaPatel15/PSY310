...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\HP\\OneDrive\\Documents\\Psychopy_Experiments_Tutorials\\solo_project\\untitled_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instructions = visual.TextStim(win=win, name='instructions',
        text="If the cue is 'x', then:-\nPress 's' if the shape is square\nPress 'c' if the shape is circle\n\nIf the cue is 'y', then:-\nPress 'r' if the colour is red\nPress 'b' if the colour is blue\n\nClick 'space' to start!",
        font='Arial',
    ...
    instructions_2Components = [instructions, start_resp]
    ...
    for thisTrial in trials:
        ...
        polygon.setVertices(stimulus)
        # create starting attributes for key_resp
        key_resp.keys = []
        key_resp.rt = []
        _key_resp_allKeys = []
        # keep track of which components have finished
        trialComponents = [fixation, text_cue, polygon, key_resp]
        ...
        while continueRoutine:
            ...
            if text_cue.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
            ...
            if text_cue.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > text_cue.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if key_resp.status == STARTED and not waitOnFlip:
                theseKeys = key_resp.getKeys(keyList=['s', 'c', 't', 'a', 'r', 'b', 'g', 'y'], ignoreKeys=["escape"], waitRelease=False)
